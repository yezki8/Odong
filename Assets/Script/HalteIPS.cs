...
public class HalteIPS : MonoBehaviour
{
    public MoveOnPath OdongIPS;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.transform.tag == "odongips")
        {
            turun = Random.Range(0, OdongIPS.penumpang);
            berhenti = true;
            print("berhenti");
            OdongIPS.speed = 0;
            SadEmoji.SetActive(false);
            ...
            if (OdongIPS.penumpang != 0)
            {
                OdongIPS.penumpang -= turun;
                OdongIPS.sisaKursi = 20 - OdongIPS.penumpang;
            }
            if (OdongIPS.sisaKursi >= penunggu)
            {
                orangNaik = penunggu;
                OdongIPS.penumpang += orangNaik;
                OdongIPS.sisaKursi = 20 - OdongIPS.penumpang;
                penunggu = 0;
            }
            else if (OdongIPS.sisaKursi < penunggu)               // jika penunggu lebis banyak dari sisa kursi
            {
                orangNaik = OdongIPS.sisaKursi;
                penunggu = penunggu - OdongIPS.sisaKursi;           //Jika sisa kursi lebih sedikit dari penunggu di halte
                OdongIPS.penumpang += orangNaik;
                OdongIPS.sisaKursi = 20 - OdongIPS.penumpang;       //Sisa kursi disesuaikan dengan penumpang
            }
